...
import { VerkaeuferViewDialog } from '@/components/dialogs/VerkaeuferViewDialog';
...
const ARTIKEL_FIELDS = [
  { key: 'verkaeufer', label: 'Verkäufer', type: 'applookup/select', targetEntity: 'verkaeufer', targetAppId: 'VERKAEUFER', displayField: 'vorname' },
  { key: 'kategorie', label: 'Kategorie', type: 'applookup/select', targetEntity: 'kategorien', targetAppId: 'KATEGORIEN', displayField: 'kategoriename' },
  { key: 'titel', label: 'Titel', type: 'string/text' },
  { key: 'beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'zustand', label: 'Zustand', type: 'lookup/select', options: [{ key: 'neu', label: 'Neu' }, { key: 'wie_neu', label: 'Wie neu' }, { key: 'sehr_gut', label: 'Sehr gut' }, { key: 'gut', label: 'Gut' }, { key: 'akzeptabel', label: 'Akzeptabel' }, { key: 'defekt', label: 'Defekt' }] },
...
const VERKAEUFER_FIELDS = [
  ...
  { key: 'zusatzinfo', label: 'Zusätzliche Informationen', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'artikel', label: 'Artikel', pascal: 'Artikel' },
  { key: 'kategorien', label: 'Kategorien', pascal: 'Kategorien' },
  { key: 'verkaeufer', label: 'Verkäufer', pascal: 'Verkaeufer' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('artikel');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'verkaeufer': return (data as any).verkaeufer as Verkaeufer[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'artikel':
        lists.verkaeuferList = (data as any).verkaeufer ?? [];
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'artikel' && fieldKey === 'verkaeufer') {
      const match = (lists.verkaeuferList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'artikel' && fieldKey === 'kategorie') {
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'verkaeufer': return VERKAEUFER_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'verkaeufer': return {
        create: (fields: any) => LivingAppsService.createVerkaeuferEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateVerkaeuferEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteVerkaeuferEntry(id),
      };
      default: return null;
    }
  }, []);
    ...
    >
      ...
      {(createEntity === 'artikel' || dialogState?.entity === 'artikel') && (
        <ArtikelDialog
          ...
          verkaeuferList={(data as any).verkaeufer ?? []}
      ...
      {(createEntity === 'verkaeufer' || dialogState?.entity === 'verkaeufer') && (
        <VerkaeuferDialog
          open={createEntity === 'verkaeufer' || dialogState?.entity === 'verkaeufer'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'verkaeufer' ? handleUpdate : (fields: any) => handleCreate('verkaeufer', fields)}
          defaultValues={dialogState?.entity === 'verkaeufer' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Verkaeufer']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Verkaeufer']}
        />
      )}
      ...
      {viewState?.entity === 'verkaeufer' && (
        <VerkaeuferViewDialog
          open={viewState?.entity === 'verkaeufer'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'verkaeufer', record: r }); }}
        />
      )}
