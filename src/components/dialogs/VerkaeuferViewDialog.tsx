...
interface VerkaeuferViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: Verkaeufer | null;
  onEdit: (record: Verkaeufer) => void;
}

export function VerkaeuferViewDialog({ open, onClose, record, onEdit }: VerkaeuferViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Verkäufer anzeigen</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm whitespace-pre-wrap">{record.fields.zusatzinfo ?? '—'}</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
