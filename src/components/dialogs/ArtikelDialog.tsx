...
export function ArtikelDialog({ open, onClose, onSubmit, defaultValues, verkaeuferList, kategorienList, enablePhotoScan = true, enablePhotoLocation = true }: ArtikelDialogProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'artikel');
      await onSubmit(clean as Artikel['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="verkaeufer" entity="Verkäufer">\n${JSON.stringify(verkaeuferList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      ...
      const schema = `{\n  "verkaeufer": string | null, // Display name from Verkäufer (see <available-records>)\n  "kategorie": string | null, // Display name from Kategorien (see <available-records>)\n  "titel": string | null, // Titel\n  "beschreibung": string | null, // Beschreibung\n  "zustand": LookupValue | null, // Zustand (select one key: "neu" | "wie_neu" | "sehr_gut" | "gut" | "akzeptabel" | "defekt") mapping: neu=Neu, wie_neu=Wie neu, sehr_gut=Sehr gut, gut=Gut, akzeptabel=Akzeptabel, defekt=Defekt\n  "preis": number | null, // Preis in Euro\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["verkaeufer", "kategorie"]);
  ...
  const DIALOG_INTENT = defaultValues ? 'Artikel bearbeiten' : 'Artikel hinzufügen';
