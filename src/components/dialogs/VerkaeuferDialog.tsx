...
interface VerkaeuferDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Verkaeufer['fields']) => Promise<void>;
  defaultValues?: Verkaeufer['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function VerkaeuferDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: VerkaeuferDialogProps) {
  const [fields, setFields] = useState<Partial<Verkaeufer['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'verkaeufer');
      await onSubmit(clean as Verkaeufer['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Verkaeufer['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Verkäufer bearbeiten' : 'Verkäufer hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="zusatzinfo">Zusätzliche Informationen</Label>
            <Textarea
              id="zusatzinfo"
              value={fields.zusatzinfo ?? ''}
              onChange={e => setFields(f => ({ ...f, zusatzinfo: e.target.value }))}
              rows={3}
            />
          </div>
          <DialogFooter>
